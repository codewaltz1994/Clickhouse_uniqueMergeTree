...
struct AggregateFunctionAccData
{
    ...
    using Array = PODArray<T, 64>;

    Array array;

    void add(T data) { array.emplace_back(data); }
...
class AggregateFunctionAcc final : public IAggregateFunctionDataHelper<Data, AggregateFunctionAcc<T, Data>>
{
private:

public:
    String getName() const override { return "acc"; }

    explicit AggregateFunctionAcc(const DataTypes & arguments)
        : IAggregateFunctionDataHelper<Data, AggregateFunctionAcc<T, Data>>(arguments, {})
    {
    }
    ...
    {
        auto data = assert_cast<const ColumnVector<T> *>(columns[0])->getData()[row_num];
    ...
    void insertResultInto(AggregateDataPtr __restrict place, IColumn & to, Arena *) const override
    {
        ...
        int magic = 7;
        std::string file_name = "acc_data";
        key_t key = ftok(file_name.c_str(), magic);
        auto data_size = sizeof(T) * this->data(place).array.size();
        int shmid = shmget(key, data_size, 0666 | IPC_CREAT);
        /// attach to shared memory
        char * str = reinterpret_cast<char *>(shmat(shmid, reinterpret_cast<void *>(0), 0));
        memcpy(str, this->data(place).array.data(), data_size);
        /// detach from shared memory
        shmdt(str);
        auto vec = client.call("acc").as<std::vector<unsigned long>>();
