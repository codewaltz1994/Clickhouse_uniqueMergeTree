...
BlocksWithPartition MergeTreeDataWriter::splitBlockIntoParts(
    const Block & block, size_t max_parts, const StorageMetadataPtr & metadata_snapshot, ContextPtr context, bool has_delete_op)
{
    ...
    UniqueMergeTreeWriteState & write_state)
{
    ...
    for (size_t i = 0; i < sort_columns_size; ++i)
        sort_description.emplace_back(block.getPositionByName(unique_column_names[i]), 1, 1);
