...
StorageBinary::StorageBinary(const StorageID & storage_id_, const std::string & user_files_path, size_t max_number_)
    : IStorage(storage_id_), file_path(user_files_path), max_number(max_number_)
{
    StorageInMemoryMetadata storage_metadata;
    storage_metadata.setColumns(ColumnsDescription(
        {{"data", std::make_shared<DataTypeString>()},
         {"device", std::make_shared<DataTypeUInt8>()},
         {"frame", std::make_shared<DataTypeUInt32>()}}));
...
public:
    ...
    BinarySource(const StorageBinary & storage_, const StorageMetadataPtr & metadata_snapshot_, Names, size_t max_number_)
        : SourceWithProgress(metadata_snapshot_->getSampleBlockNonMaterialized()), path(storage_.path()), max_number(max_number_)
    {
    }
    ...
    Chunk generate() override
    {
        ...
        auto device_col = DataTypeUInt8{}.createColumnConst(1, device);
        columns.emplace_back(std::move(device_col));
...
private:
    // FIXME
    // std::shared_ptr<std::string> buf = nullptr;
    UInt8 device = 0;
	UInt64 current = 0;
    std::string path;
	UInt64 max_number;
...
{
    return Pipe(std::make_shared<BinarySource>(*this, metadata_snapshot, column_names, max_number));
}
...
void registerStorageBinary(StorageFactory & factory)
{
    factory.registerStorage("Binary", [](const StorageFactory::Arguments & factory_args) {
        ...
        if (engine_args_ast.size() != 1) // NOLINT
            throw Exception(
