...
int main()
{
    ...
    for (;;)
    {
        ...
        size_t max_comressed_size = CpuLZ4::getMaxCompressedDataSize(message_data.size());

        std::string compress_data;
        compress_data.resize(max_comressed_size);

        Stopwatch sw;
        size_t compressed_size = CpuLZ4::compress((char *)message_data.data(), message_data.size(), (char *)compress_data.data());
        std::cout << "uncompressed_size = " << message_data.size() / 1024.0 / 1024.0
        ...
        std::cout << "batch size = " << batch_size << ", compress ratio = " << (double)message_data.size() / compressed_size << std::endl;

        // send the reply to the client
        socket.send(zmq::buffer(compress_data.data(), compressed_size), zmq::send_flags::sndmore);
